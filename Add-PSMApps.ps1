...
switch ($Application) {
    ...
    "MicrosoftEdgeX86" {
        ...
        $AppLockerEntries = @(
            (New-PSMApplicationElement -Xml $xml -EntryType Application -Name MicrosoftEdge -FileType Exe -Path $EdgePath -Method Publisher)
            (New-PSMApplicationElement -Xml $xml -EntryType Application -Name MicrosoftEdgeDriver -FileType Exe -Path $EdgeDriverPath -Method Hash)
        )
        Add-PSMConfigureAppLockerSection -SectionName "Microsoft Edge" -XmlDoc ([REF]$xml) -AppLockerEntries $AppLockerEntries
    }
}
